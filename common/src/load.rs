...
pub fn string() -> String {
    ...
    // Load data
    read_to_string(&path).unwrap_or_else(|_| panic!("Could not read the file \"{}\"", path))
}
...
pub fn lines() -> Vec<String> {
    ...
    // Load data
    read_lines(&path).unwrap_or_else(|_| panic!("Could not read the file \"{}\"", path))
}
...
fn read_lines(filename: impl AsRef<Path>) -> io::Result<Vec<String>> {
    read_to_string(filename).map(|input| input.lines().map(str::to_string).collect())
}
...
fn read_comma_separated_values(filename: impl AsRef<Path>) -> io::Result<Vec<String>> {
    read_to_string(filename).map(|input| input.split(',').map(|s| s.trim().to_string()).collect())
}
...
pub fn map() -> Vec<Vec<char>> {
    ...
    // Load data
    read_map(&path).unwrap_or_else(|_| panic!("Could not read the file \"{}\"", path))
}
...
fn read_map(filename: impl AsRef<Path>) -> io::Result<Vec<Vec<char>>> {
    read_to_string(filename).map(|input| input.lines().map(|line| line.chars().collect()).collect())
}
...
pub fn numbers_map() -> Vec<Vec<i32>> {
    ...
    // Load data
    read_numbers_map(&path).unwrap_or_else(|_| panic!("Could not read the file \"{}\"", path))
}
...
mod tests {
    ...
    fn test_read_lines() {
        ...
        let lines = read_lines(&file_path).expect("Failed to read lines");
